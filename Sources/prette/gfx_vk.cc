...
void InitFramebuffers(const Driver* driver, const VkRenderPass& pass, const VkExtent2D& extent,
  ...
  ASSERT(driver);
  const auto num_images = SwapChain::GetNumberOfImages();
  ...
  for (auto idx = 0; idx < num_images; idx++) {
    ...
    create_info.pAttachments = &SwapChain::GetImageView(idx);
...
CommandBufferPool::CommandBufferPool(const VkCommandPool& pool, const uint64_t num_buffers) {
  ...
  Driver::Get()->AllocateCommandBuffers(alloc_info, &buffers_[0]);
}

CommandBufferPool::~CommandBufferPool() {
  if (!IsEmpty())
    Driver::Get()->ReleaseCommandBuffers(buffers_);
}
...
void RenderPassScope::Bind(BaseRenderPipeline* p) {
  ASSERT(p);
  vkCmdBindPipeline(buffer_, VK_PIPELINE_BIND_POINT_GRAPHICS, p->GetPipeline());
}
...
void Driver::CreatePipelineLayout(const VkPipelineLayoutCreateInfo& create_info, VkPipelineLayout& layout) {
  CHECK_VK(FATAL, vkCreatePipelineLayout(GetDevice(), &create_info, GetAllocator(), &layout),
           "failed to create VkPipelineLayout");
}

void Driver::CreateRenderPass(const VkRenderPassCreateInfo& create_info, VkRenderPass& pass) {
  CHECK_VK(FATAL, vkCreateRenderPass(GetDevice(), &create_info, GetAllocator(), &pass), "failed to create VkRenderPass");
}

void Driver::CreatePipelineCache(const VkPipelineCacheCreateInfo& create_info, VkPipelineCache& cache) {
  CHECK_VK(FATAL, vkCreatePipelineCache(GetDevice(), &create_info, GetAllocator(), &cache), "failed to create VkPipelineCache");
}

void Driver::CreateGraphicsPipeline(const VkGraphicsPipelineCreateInfo& create_info, const VkPipelineCache& cache,
                                    VkPipeline& pipeline) {
  CHECK_VK(FATAL, vkCreateGraphicsPipelines(GetDevice(), cache, 1, &create_info, GetAllocator(), &pipeline),
           "failed to create vk pipeline");
}

void Driver::AllocateCommandBuffers(const VkCommandBufferAllocateInfo& alloc_info, VkCommandBuffer* buffers) {
  CHECK_VK(FATAL, vkAllocateCommandBuffers(GetDevice(), &alloc_info, buffers), "failed to allocate VkCommandBuffers");
}
