...
void GuiRenderer::InitFramebuffers(const Driver* driver) {
  ASSERT(driver);
  vk::InitFramebuffers(driver, pass_, SwapChain::GetExtent(), framebuffers_);
}
...
void GuiRenderer::Draw(SwapChainFrame* frame, std::vector<VkCommandBuffer>& cmd_buffers) {
  // clang-format off
  ...
  {
    RenderPassScope render_pass(buffer, pass_, framebuffers_[frame->GetImage()], kClearValues);
