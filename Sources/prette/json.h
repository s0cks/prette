...
static inline bool ParseJson(const uri::basic_uri& uri, Document& doc) {
  return ParseJson(uri::Uri(uri), doc);
}

bool ParseRawJson(const char* json, Document& doc);

static inline bool ParseRawJson(const std::string& json, Document& doc) {
...
static inline rx::observable<DocumentPtr> ParseDocument(const uri::Uri& uri, const int buffer_size = kDefaultJsonFileBufferSize) {
...
static inline std::string GetParseError(const Document& doc) {
...
static inline bool Validate(SchemaDocument& schema, Document& doc) {
...
static inline bool Validate(SchemaDocument& schema, const std::string& filename) {
  Document doc;
...
static inline bool Validate(const std::string& schemaf, const std::string& filename) {
  Document schemad;
...
class JsonSpec {
 protected:
  ...
  JsonSpec(const uri::Uri& uri, DocumentPtr doc) :
    ...
    doc_(doc) {
    ASSERT(doc.get());
    ASSERT(uri.HasScheme("file"));
    ASSERT(uri.HasExtension(".json"));
  }
  ...
  inline std::optional<std::string> GetStringProperty(const char* name) const {
  ...
  inline std::optional<uri::Uri> GetUriProperty(const char* name) const {
...
class ReaderHandlerTemplate : public BaseReaderHandler<UTF8<>, D> {
 protected:
  ...
  explicit ReaderHandlerTemplate(const std::string& type, const State init_state = State::kClosed) :
...
class JsonParseResult {
 ...
 public:
  ...
  bool operator==(const JsonParseResult& rhs) const {
    return success_ == rhs.success_ && message_ == rhs.message_;
  }
...
template <class H>
static inline JsonParseResult ParseJson(const uri::Uri& uri, H& handler, const uword buffer_size = kDefaultJsonFileBufferSize) {
  ...
  if (!reader.Parse(frs, handler)) {
    ...
    const auto error =
        fmt::format("failed to parse json from file {0:s}: {1:s} at {2:d}", uri, json::GetParseError_En(code), offset);
    fclose(file);
