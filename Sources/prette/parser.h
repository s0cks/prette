...
struct TokenTemplate {
 ...
 public:
  ...
  TokenTemplate(const Kind k, const Position& p) :
    ...
    data(nullptr),
    length(0) {}
  TokenTemplate(const Kind k, const uint64_t r, const uint64_t c) :
    TokenTemplate(k, Position(r, c)) {}
  TokenTemplate(const Kind k, const Position& p, uint8_t* t, const uint64_t l) :
    kind(k),
    pos(p),
    data(t),
    length(l) {}
...
template <const uint64_t ParserBufferSize, const uint64_t TokenBufferSize>
