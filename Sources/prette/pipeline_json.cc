...
#include "prette/render_pass.h"  // IWYU pragma: keep
...
auto PipelineHandler::OnParseVertexClass(const std::string& value) -> bool {
  if (EqualsIgnoreCase(value, "default")) {
    builder_->AddVertexBinding(Vertex::GetBindingDescription());
    builder_->AddVertexAttributes(Vertex::GetAttributeDescriptions());
    return TransitionTo(PipelineHandlerState::kOpenDoc);
  } else if (EqualsIgnoreCase(value, "tile") || EqualsIgnoreCase("color2d", value) ||
             EqualsIgnoreCase("color2d", value)) {
