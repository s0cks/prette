...
class SipHashDigest {
  ...
  static inline auto RotateLeft(const uword val, const int32_t cnt) -> uword {
    // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers)
    return (val << cnt) | (val >> (64 - cnt));
  }

  static inline auto Read8(const Key& key, const uword o = 0) -> uword {
    // NOLINTBEGIN(cppcoreguidelines-avoid-magic-numbers)
    return static_cast<uword>(key[o + 0]) | (static_cast<uword>(key[o + 1]) << 8) |
           (static_cast<uword>(key[o + 2]) << 16) | (static_cast<uword>(key[o + 3]) << 24) |
           (static_cast<uword>(key[o + 4]) << 32) | (static_cast<uword>(key[o + 5]) << 40) |
           (static_cast<uword>(key[o + 6]) << 48) | (static_cast<uword>(key[o + 7]) << 56);
 ...
 public:
  SipHashDigest(const Key& key, const uword c, const uword d) :
    c_(c),
    d_(d) {
