...
namespace prt {
static ThreadLocal<Settings> settings_{};
...
void Settings::Save() {
  ...
  {
    topic_.PublishSavingSettingsEvent();
    // TODO: generate write batch for batching saves
    topic_.PublishSettingsSavedEvent();
  }
...
void Settings::Load() {
  PRT_PROFILING_BEGIN(loading);
  {
    ...
    topic_.PublishLoadingSettingsEvent();
    ...
    db_ = db;
    if (is_genesis)
      topic_.PublishGenerateSettingsEvent();
    topic_.PublishSettingsLoadedEvent();
  }
