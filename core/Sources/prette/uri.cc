...
auto Uri::TryParse(uri::Uri& result, const basic_uri& uri, const uri::Scheme& default_scheme) -> bool {
  Parser::Config config = {
...
auto Uri::HasScheme(const Scheme& a) const -> bool {
  return HasScheme() && EqualsIgnoreCase(scheme, a);
}

template <class Container>
auto Has(const Scheme& value, const Container& values) -> bool {
  return !value.empty() && std::find(std::begin(values), std::end(values), value) != std::end(values);
}
...
static inline auto IsValidUri(const basic_uri& value, const Container& schemes) -> bool {
  ASSERT(!value.empty());
