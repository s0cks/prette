...
class ParseResult {
 ...
 public:
  static inline auto Success(const std::string& message = "Success.") -> ParseResult {
    return {true, message};
  }

  static inline auto Failure(const std::string& message) -> ParseResult {
...
class Parser : public ParserTemplate<kDefaultParserBufferSize, kDefaultTokenBufferSize> {
 ...
 public:
  static inline constexpr auto NoFlags() -> Config::Flags {
    return Config::kNoFlags;
  }

  static inline constexpr auto Strict(const bool value = true) -> Config::Flags {
    return NoFlags() | Config::StrictField::Encode(value);
  }

  static inline constexpr auto ParseFragments(const bool value = true) -> Config::Flags {
    return NoFlags() | Config::ParseFragmentsField::Encode(value);
  }

  static inline constexpr auto ParseQueries(const bool value = true) -> Config::Flags {
    return NoFlags() | Config::ParseQueriesField::Encode(value);
  }

  static inline constexpr auto DefaultFlags() -> Config::Flags {
    return Strict(true) | ParseFragments(false) | ParseQueries(false);
  }
 ...
 private:
  ...
  static inline auto IsQueryDelimiter(const char c) -> bool {
    switch (c) {
      case '&':
      case ';':
        return true;
  ...
  static inline auto IsValidQueryKeyChar(const char c) -> bool {
    switch (c) {
      case '#':
      case '=':
      case '\0':
  ...
  inline auto CallIfExists(Func* func, Args... args) const -> bool {
  ...
  inline auto OnParseQuery1(const uint64_t idx, const char* key, const uword key_len, const char* value,
                            const uword value_len) const -> bool {
    return CallIfExists(config_.OnParseQuery1, idx, key, key_len, value, value_len);
  }

  inline auto OnParseFragment(const char* fragment, const uint64_t length) const -> bool {
    return CallIfExists(config_.OnParseFragment, fragment, length);
  }

  inline auto OnParseError() const -> bool {
