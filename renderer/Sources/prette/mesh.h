...
class SingleMesh {
 ...
 private:
  VertexBufferType* vertex_buffer_ = nullptr;
  IndexBufferType* index_buffer_ = nullptr;

 public:
  SingleMesh() = default;
  SingleMesh(const VertexArray& vertices, const IndexArray& indices) :
    vertex_buffer_(new VertexBufferType(vertices)),
    index_buffer_(new IndexBufferType(indices)) {
  ...
  auto IsInitialized() const -> bool {
    return vk::AllInitialized(vertex_buffer_, index_buffer_);
  }

  auto IsDirty() const -> bool {
    return GetVertexBuffer()->IsDirty() || GetIndexBuffer()->IsDirty();
  }
