...
class PipelineHandler : public BaseStatefulReaderHandler<PipelineHandlerState, PipelineHandler> {
 ...
 private:
  ...
  void OnParseShaderStage(const VkShaderStageFlagBits rhs) {
    builder_->WithShaderStage(rhs);
  }

  void OnParseShaderStage(const std::string& rhs) {
    if (EqualsIgnoreCase("vertex", rhs)) {
      builder_->WithShaderStage(VK_SHADER_STAGE_VERTEX_BIT);
    } else if (EqualsIgnoreCase("fragment", rhs)) {
      builder_->WithShaderStage(VK_SHADER_STAGE_FRAGMENT_BIT);
    }
  }
